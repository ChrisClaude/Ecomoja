...
const ToggleShopMobility = ()=>{
	...
	const [isShopMobility, setShopMobility] = React.useState<boolean>(false);
	...
	React.useEffect(()=>{
		function ToggleShoppingMobility(){
			if(shopMobility){
				...
				setShopMobility(shopMobility);
			}
			if(isShopMobility && !shopMobility){
				router.push('/');
			}
		}
		ToggleShoppingMobility();
	}, [dispatch ,isShopMobility, shopMobility]);

	React.useEffect(()=>{
		if(router.pathname === '/mobility'){
			shopMobilityIsChecked(true);
		}
		else{
...
<input id="Toggle4" checked={shopMobility} type="checkbox" onChange={(e)=>{shopMobilityIsChecked(e.target.checked)}} className="peer" hidden/>
